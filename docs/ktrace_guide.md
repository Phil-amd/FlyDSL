# KTRACE — In-Kernel Event Tracing
...
Either call **consumes** the trace: it reads the records back and re-arms the buffer for
the next launch. That is what keeps a repeated trace -- a benchmark loop, an autotune
sweep -- from accumulating claims until the buffer overflows. Two consequences: call
`summary()` *before* `collect()` if you want both from one launch, and expect `None` from
a second call with no launch in between.
...
| `collect(kernel, out_dir=None) -> path` | Write the trace (host side); consumes it |
| `summary() -> dict` | Per-phase totals (host side); consumes it |
...
| `FLYDSL_KTRACE_EVENTS_PER_WAVE` | `256` | Slots reserved per wave |
...
- A wave emitting more events than `FLYDSL_KTRACE_EVENTS_PER_WAVE` stops recording rather
