...
kernels as a trailing implicit argument (see :mod:`flydsl.expr.rocdl.ktrace_emit` and
...
class TraceBuffer:
    ...
    def claimed_slots(self) -> int:
        ...
        Every wave that reaches the prologue claims ``FLYDSL_KTRACE_EVENTS_PER_WAVE`` slots
...
def decode_records(raw: bytes) -> list[dict]:
    ...
    per_wave = int(env.ktrace.events_per_wave)
    out = []
    for off in range(0, len(raw) - _ktrace.RECORD_BYTES + 1, _ktrace.RECORD_BYTES):
        ...
        if per_wave > 0 and (abs_slot - 1) % per_wave == 0:
            continue  # this wave's counter word
