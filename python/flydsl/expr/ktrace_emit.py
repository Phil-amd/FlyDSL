...
def emit_record(event_name: str | None, kind: int, *, payload: Any = None, start_slot=None):
    ...
    st.next_index += 1  # call sites, for diagnostics only
    ...
    with ir.InsertionPoint(if_op.regions[0].blocks[0]):
        ...
        within_wave = _arith.CmpIOp(_arith.CmpIPredicate.slt, local, per_wave).result
        ...
        # bounds guard too. A suppressed store yields the unoffset slot; it pairs with
        # nothing, which is correct -- no record was written.
    ...
    with ir.InsertionPoint(if_op.regions[1].blocks[0]):
        # Non-leader lanes yield the wave base; they never pair a token themselves.
        scf.YieldOp([st.base])
