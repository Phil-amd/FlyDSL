...
def merge_names(names: dict[str, int]) -> None:
    ...
    any binding this process assigned.  ``event_id`` then allocates above every id in use,
    so a name assigned after a merge can never reuse a restored id and alias it.
    """
    _NAMES.update(names)
