...
"""KTRACE -- in-kernel event tracing for gfx942/gfx950.
...
``flydsl.utils.ktrace_runtime``.  Each wave claims its slot range once at kernel
...
# Records of the last traced launch, read back once and reused. Lets collect() and
# summary() both describe the SAME launch: the device read is destructive, so without
# this the second call saw an empty buffer -- contradicting the documented workflow.
...
def sentinel_token(event_name: str) -> RangeToken:
    ...
    Use when a ``range_end`` precedes the ``range_start`` it pairs with in source order::
        ...
        tok = fx.ktrace.sentinel_token("mma_k_tile")
        for k_tile in range(0, k_tiles, 1):
            fx.ktrace.range_end(tok)          # no-op on the first iteration
            tok = fx.ktrace.range_start("mma_k_tile")
            ...
        fx.ktrace.range_end(tok)
    """
    if not _enabled():
        return _disabled_token(event_name)
    return RangeToken(_i32(0).ir_value(), _i1_const(True), event_name)
...
def _emit_record(event_name, kind, *, payload=None, start_slot=None, guard=None):
    ...
    from .rocdl.ktrace_emit import emit_record
...
def _take_records() -> list | None:
    """Read the records of the last traced launch and re-arm the buffer.
    ...
    The records are cached host-side, so :func:`collect` and :func:`summary` can both be
    called after one launch -- the device read happens once, and the second caller is
    served from the cache.  The cache is dropped when the next launch re-arms the buffer.
    ...
    # The cache is valid only while the buffer is still empty, i.e. nothing has launched
    # since the read that filled it. The cursor is the evidence, and asking costs one
    # 4-byte copy: there is no per-launch host hook to invalidate on -- which is why the
    # read was destructive to begin with. A non-zero cursor means a newer launch recorded,
    # so the cache is stale and the device becomes the source of truth again.
    if _CACHED_RECORDS is not None:
        if buf.claimed_slots() == 0:
    ...
    try:
        ...
        # Also on the overflow path: leaving a full buffer behind would make every later
        # call raise the same error, hiding whichever launch actually overflowed.
        buf.reset()
...
def collect(kernel: str = "kernel", out_dir=None) -> str | None:
    ...
    before) and both describe the same launch.
    ...
    rather than trimmed.
    """
    from ..utils import ktrace_runtime, ktrace_trace
    from .rocdl import ktrace_emit
    ...
    return ktrace_trace.write_trace(
        ...
        kernel=kernel,
        out_dir=out_dir or ktrace_runtime.dump_dir(),
    )
