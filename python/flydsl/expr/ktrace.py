...
# Records of the last traced launch, held until the next launch drops them (the jit layer
# calls _invalidate_ktrace_records). Lets collect() and summary() both describe the SAME
...
def _take_records() -> list | None:
    ...
    # The cache is dropped by the jit layer on every launch (_invalidate_ktrace_records),
    ...
    try:
        ...
        # finally would swap the actionable "claimed N slots, buffer holds M, re-run with
        # FLYDSL_KTRACE_BUFFER_BYTES>=..." for a bare HIP error code, precisely when the
