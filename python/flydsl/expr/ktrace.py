...
def _emit_record(event_name, kind, *, payload=None, start_slot=None, guard=None):
    ...
    from .ktrace_emit import emit_record
...
def collect(kernel: str = "kernel", out_dir=None) -> str | None:
    ...
    Consumes the trace: the buffer is re-armed for the next launch, so calling this
    twice without an intervening launch returns None the second time.  Use
    :func:`summary` *before* :func:`collect` when both are wanted from one launch.
...
def summary() -> dict | None:
    ...
    Consumes the trace, exactly as :func:`collect` does.
    """
