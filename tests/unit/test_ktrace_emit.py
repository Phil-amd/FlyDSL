...
def test_disabled_build_is_identical_to_uninstrumented(monkeypatch):
    ...
    for artifact in ("s.memrealtime", "s_memrealtime", "atomicrmw", "__ktrace"):
...
def test_compile_hint_packs_the_buffer_slot_it_compiled(tmp_path):
    """A ``ktrace`` compile hint must reach the ABI packing, not just the emitter.
...
def test_range_end_without_a_start_is_dropped():
    ...
    records = ktrace_runtime.decode_records(raw)
...
def test_buffer_bytes_invalidates_the_cache(monkeypatch):
    ...
    """
    baked = {
        # env var -> the emitter value it is compiled into
        "FLYDSL_KTRACE_ENABLE": "tracing on/off changes the whole kernel body",
        "FLYDSL_KTRACE_EVENTS_PER_WAVE": "per-wave slot bound constant",
...
def test_events_per_wave_cannot_silence_the_trace():
    """events_per_wave=1 emits `local < 0`, suppressing every record; 2 is the minimum."""
    from flydsl.utils import env

    option = type(env.ktrace).__dict__["events_per_wave"]
    assert option.min_value >= 2, (
        "events_per_wave=1 makes the store guard `local < 0`, which is false for the very "
        "first event: a clean run and a silently empty trace"
    )
...
def test_colliding_event_ids_are_rejected(monkeypatch):
    ...
    assert ktrace_emit.event_id("epilogue") == 1
